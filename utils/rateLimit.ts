...
/**
 * Checks whether the specified user is permitted to perform an upload under the configured upload rate limiter.
 *
 * @param userId - The identifier of the user to check
 * @returns An object with `allowed` set to `true` if the upload is permitted (or if no limiter is configured), and `reset` indicating when the rate-limit window resets according to the limiter
 */
